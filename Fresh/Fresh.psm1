...
function BestPriorityForeground {
	...
	New-ItemProperty -Path "HKLM:\System\CurrentControlSet\Control\Session Manager\kernel" -Name DistributeTimers -PropertyType DWord -Value 1 -Force
	...
	New-ItemProperty -Path "HKLM:\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Multimedia\SystemProfile\Tasks\Games" -Name "GPU Priority" -PropertyType DWord -Value 12 -Force
...
function SvcHostSplitThresholdInKB {
	...
	New-ItemProperty -Path HKLM:\SYSTEM\CurrentControlSet\Control -Name SvcHostSplitThresholdInKB -PropertyType DWord -Value 34537040 -Force
}
...
function PrioritizeCSRSSService {
	...
	New-ItemProperty -Path "HKLM:\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Image File Execution Options\dwm.exe\PerfOptions" -Name CpuPriorityClass -Type "DWORD" -Value "4" -Force
	...
	New-ItemProperty -Path "HKLM:\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Image File Execution Options\winlogon.exe\PerfOptions" -Name CpuPriorityClass -Type "DWORD" -Value "4" -Force
...
function ChocolateyPackageManager {
	[System.Net.ServicePointManager]::SecurityProtocol = [System.Net.ServicePointManager]::SecurityProtocol -bor 3072; Invoke-Expression ((New-Object System.Net.WebClient).DownloadString('https://chocolatey.org/install.ps1')); choco feature enable -n=allowGlobalConfirmation; choco feature enable -n useFipsCompliantChecksums; choco feature enable -n=useEnhancedExitCodes; choco config set commandExecutionTimeoutSeconds 14400; choco config set --name="'cacheLocation'" --value="'C:\temp\chococache'"; choco config set --name="'proxyBypassOnLocal'" --value="'true'"; cinst pswindowsupdate dotnetfx directx vcredist-all jre8 openal xna; Get-WindowsUpdate -NotCategory "Upgrades", "Silverlight" -NotTitle Preview -MicrosoftUpdate -Install -AcceptAll -IgnoreReboot -Verbose;	cinst --ignore-checksums pswindowsupdate dotnetfx directx vcredist-all jre8 openal xna; Get-WindowsUpdate -NotCategory "Upgrades", "Silverlight" -NotTitle Preview -MicrosoftUpdate -Install -AcceptAll -IgnoreReboot -Verbose; cinst 7zip.install notepadplusplus.install cpu-z.install teracopy jpegview potplayer transmission-qt spotify; cinst --ignore-checksums 7zip.install notepadplusplus.install cpu-z.install teracopy jpegview potplayer transmission-qt spotify
}
